...
import cl from './NextRunkUp.module.css';
...
const NextRunkUp = () => {
    ...
    const handleSubmit = async () => {
        if (startDate && endDate) {
          try {
            setLoading(true);
            const response = await axios.get(`http://127.0.0.1:8000/api/v1/rank-up-list/?startDate=${startDate}&endDate=${endDate}`);            setData(response.data.data);
    ...
    const handleDownloadExcel = () => {
        if (startDate && endDate) {
            window.location.href = `http://127.0.0.1:8000/api/v1/rank-up-list-download/?startDate=${startDate}&endDate=${endDate}`;
        } else {
