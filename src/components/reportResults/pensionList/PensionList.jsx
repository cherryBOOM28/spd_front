...
const PensionList = () => {
    ...
    const handleSubmit = async () => {
        if (startDate && endDate) {
          try {
            setLoading(true);
            const response = await axios.get(`http://127.0.0.1:8000/api/v1/pension-list-view/?startDate=${startDate}&endDate=${endDate}`);
    ...
    const handleDownloadExcel = () => {
        if (startDate && endDate) {
            window.location.href = `http://127.0.0.1:8000/api/v1/pension-list-download/?startDate=${startDate}&endDate=${endDate}`;
        } else {
