...
function Vacation() {
    ...
    const [formDataVacationReview, setFormDataVacationReview] = useState({
        personId: '',
        decreeDate: '',
        startDate: '',
        endDate: '',
    ...
    const handleFormSubmitVacationReview = async () => {
        try {
            if (!formDataVacationReview.personId || !formDataVacationReview.decreeDate || !formDataVacationReview.endDate || !formDataVacationReview.startDate || !formDataVacationReview.otpuskType) {
                // Show a warning notification
                NotificationManager.warning('Пожалуйста, зполните все поля!', 'Поля пустые', 3000);
                return; // Stop form submission
            };
            ...
            const accessToken = Cookies.get('jwtAccessToken');
            const response = await axios.post('http://127.0.0.1:8000/api/v1/generate-otpusk-otziv/', formDataVacationReview, {
                headers: {
                    'Authorization': `Bearer ${accessToken}`,
                },
                responseType: 'blob'
...
<Paper  elevation={3} className={cl.appointmentForm} style={{ marginTop: '80px' }}>
            ...
            <div className={cl.form}>
                ...
                <div className={cl.row}>
                    {formData.otpuskType === 'Отпуск Кратко' && (
